import type { Match, Player, ScheduleConfig, Team, Tournament } from './football-types';
...
function shuffledDifferent<T>(items: readonly T[], previous?: readonly T[]) {
  ...
  for (let tries = 0; tries < 4 && previous?.every((item, i) => item === result[i]); tries += 1) {
...
export function scheduleMetrics(teamCount: number, matchMinutes: number, breakMinutes: number, startTime: string) {
  const matchCount = (teamCount * (teamCount - 1)) / 2;
  const requiredMinutes = matchCount * (matchMinutes + breakMinutes);
  return { matchCount, requiredMinutes, endTime: addMinutes(startTime, requiredMinutes) };
}
...
function normalizedCycleOrders(team: Team, eligibleIds: string[], neededCycles: number) {
  const eligible = new Set(eligibleIds);
  const first = [...team.gkRotation.filter((id) => eligible.has(id)), ...eligibleIds.filter((id) => !team.gkRotation.includes(id))];
  const orders = team.gkCycleOrders
    .map((order) => [...order.filter((id) => eligible.has(id)), ...eligibleIds.filter((id) => !order.includes(id))])
...
export function assignGoalkeepers(tournament: Tournament): Tournament {
  ...
  const teams = tournament.teams.map((team) => ({ ...team, gkCycleOrders: team.gkCycleOrders.map((order) => [...order]) }));

  for (const team of teams) {
    const eligibleIds = team.players.filter((player) => !player.absentToday).map((player) => player.id);
    if (!eligibleIds.length) {
      for (const match of matches) {
        if (match.status !== 'finished' && match.teamAId === team.id) match.teamAGkPlayerId = undefined;
        if (match.status !== 'finished' && match.teamBId === team.id) match.teamBGkPlayerId = undefined;
      }
      continue;
    }

    const teamMatchCount = matches.filter((match) => match.teamAId === team.id || match.teamBId === team.id).length;
    const cyclesNeeded = Math.max(2, Math.ceil(teamMatchCount / eligibleIds.length) + 1);
    ...
    const dutyCounts = Object.fromEntries(eligibleIds.map((id) => [id, 0])) as Record<string, number>;
    for (const match of matches) {
      if (match.status !== 'finished') continue;
      const playerId = match.teamAId === team.id ? match.teamAGkPlayerId : match.teamBId === team.id ? match.teamBGkPlayerId : undefined;
      if (playerId && playerId in dutyCounts) dutyCounts[playerId] += 1;
    }

    for (const match of matches) {
      if (match.status === 'finished' || (match.teamAId !== team.id && match.teamBId !== team.id)) continue;
      const minimumDuty = Math.min(...eligibleIds.map((id) => dutyCounts[id]));
      const order = cycleOrders[minimumDuty] ?? cycleOrders.at(-1) ?? eligibleIds;
      const selected = order.find((id) => eligibleIds.includes(id) && dutyCounts[id] === minimumDuty) ?? eligibleIds[0];
...
export function setMatchStatus(tournament: Tournament, matchId: string, status: Match['status']) {
  const targetIndex = tournament.matches.findIndex((match) => match.id === matchId);
  ...
  const matches = tournament.matches.map((match, index) => {
    if (status === 'current') {
      ...
      if (match.status === 'current') return { ...match, status: index < targetIndex ? 'finished' as const : 'upcoming' as const };
    }
    if (match.id === matchId) return { ...match, status };
    return match;
  });
  if (status === 'finished') {
    const next = matches.find((match, index) => index > targetIndex && match.status === 'upcoming');
    if (next) next.status = 'current';
  }
  return assignGoalkeepers({ ...tournament, matches });
}

export function skipGoalkeeper(tournament: Tournament, matchId: string, teamId: string) {
  const match = tournament.matches.find((item) => item.id === matchId);
  const team = tournament.teams.find((item) => item.id === teamId);
  if (!match || !team || match.status === 'finished') return tournament;
  const currentGk = match.teamAId === teamId ? match.teamAGkPlayerId : match.teamBGkPlayerId;
  if (!currentGk) return tournament;

  const teams = tournament.teams.map((item) => {
    if (item.id !== teamId) return item;
    const moveToBack = (order: string[]) => [...order.filter((id) => id !== currentGk), currentGk];
    return { ...item, gkRotation: moveToBack(item.gkRotation), gkCycleOrders: item.gkCycleOrders.map(moveToBack) };
  });
